...
:::message alert
追記: 2021/9/6 10:30
Levelsモナドは内側のグループに順序を考慮するデータ型を使うとApplicative則, Monad則を満たさなくなってしまいます。**ここでは二重リストを使っているので以下の`Applicative`, `Monad`の実装はそれぞれの則を満たしていません**。あとがきで紹介する論文でもそうなっているように、Bagというような順序を考慮しないデータ構造を使えば則を満たしますが、Haskellで実装することはできません（例えば(Multi)Setが`Monad`のインスタンスにできないこと等）。
